...
public class Minigame extends Canvas implements KeyListener {
	...
	*/
	...
	protected void movePlayer(int dx, int dy) {
		...
		int newX = playerX;
		int newY = playerY;
		int typeAtNewLocation;
		
		// calculate new x coordinate, accounting for items in the way
		while (dx != 0) {  // true of dx is not 0
			if (dx < 0) {  // when dx is less than 0
				if (dx == -1) {
					newX = newX - 1;
					typeAtNewLocation = map[useTargetZ][newY][newX];
					dx = 0;
				} else {
					newX = newX - 1;
					typeAtNewLocation = map[useTargetZ][newY][newX];
					if (typeAtNewLocation != NOTHING) { //if there is an item at current coordinates
						dx = 0;
						newX = newX + 1;
					} else {
						dx = dx + 1;
		...
		// calculate new y coordinate, accounting for items in the way
		while (dy != 0) {
			if (dy < 0) {
				if (dy == -1) {
					newY = newY - 1;
